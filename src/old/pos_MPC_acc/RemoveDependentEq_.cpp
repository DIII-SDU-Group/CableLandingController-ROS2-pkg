  ...
  if (mTotalWorkingEq_tmp_tmp > 0) {
    ...
    for (idxDiag = 0; idxDiag < mTotalWorkingEq_tmp_tmp; idxDiag++) {
      for (idx_col = 0; idx_col <= nVar; idx_col++) {
        qrmanager->QR[idxDiag + 200 * idx_col] =
            workingset->ATwset[idx_col + 121 * idxDiag];
      }
    }
    ...
    if (nDepInd > 0) {
      ...
      while ((!exitg1) && (idx <= nDepInd - 1)) {
        double qtb;
        idxDiag = 200 * ((mTotalWorkingEq_tmp_tmp - idx) - 1);
        qtb = 0.0;
          ...
          if (idx_col <= i) {
            if ((workingset->nActiveConstr == i) || (idx_col == i)) {
              ...
              for (u0 = 0; u0 < idx_col; u0++) {
                workingset
                    ->ATwset[u0 +
                             121 * (memspace->workspace_int[idx - 1] - 1)] =
                    workingset
                        ->ATwset[u0 + 121 * (workingset->nActiveConstr - 1)];
              }
              workingset->bwset[i] =
              ...
              ix0 = workingset->nVar;
              for (u0 = 0; u0 < ix0; u0++) {
                workingset->ATwset[u0 + 121 * (idx_col - 1)] =
                    workingset->ATwset[u0 + 121 * (i - 1)];
              }
