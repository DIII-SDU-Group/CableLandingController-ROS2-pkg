              ...
              double x[120])
{
  ...
  std::memset(&solution.searchDir[0], 0, 121U * sizeof(double));
  std::copy(&x0[0], &x0[120], &solution.xstar[0]);
  CholRegManager.ldm = 200;
  ...
  WorkingSet.mConstrMax = 321;
  WorkingSet.nVar = 120;
  WorkingSet.nVarOrig = 120;
  WorkingSet.nVarMax = 121;
  WorkingSet.ldA = 121;
  std::memset(&WorkingSet.Aeq[0], 0, 9680U * sizeof(double));
  std::memset(&WorkingSet.beq[0], 0, 80U * sizeof(double));
  std::memset(&WorkingSet.lb[0], 0, 121U * sizeof(double));
  std::memset(&WorkingSet.ub[0], 0, 121U * sizeof(double));
  WorkingSet.mEqRemoved = 0;
  std::memset(&WorkingSet.indexEqRemoved[0], 0, 80U * sizeof(int));
  std::memset(&WorkingSet.ATwset[0], 0, 38841U * sizeof(double));
  WorkingSet.nActiveConstr = 0;
  std::memset(&WorkingSet.bwset[0], 0, 321U * sizeof(double));
  std::memset(&WorkingSet.maxConstrWorkspace[0], 0, 321U * sizeof(double));
  for (i = 0; i < 5; i++) {
  ...
  std::memset(&WorkingSet.isActiveConstr[0], 0, 321U * sizeof(boolean_T));
  std::memset(&WorkingSet.Wid[0], 0, 321U * sizeof(int));
  std::memset(&WorkingSet.Wlocalidx[0], 0, 321U * sizeof(int));
  for (i = 0; i < 5; i++) {
    WorkingSet.nWConstr[i] = 0;
  }
  WorkingSet.probType = 3;
  WorkingSet.SLACK0 = 1.0E-5;
  std::memset(&WorkingSet.indexLB[0], 0, 121U * sizeof(int));
  std::memset(&WorkingSet.indexUB[0], 0, 121U * sizeof(int));
  std::memset(&WorkingSet.indexFixed[0], 0, 121U * sizeof(int));
  i = 0;
  idx_global = 0;
  iATw0 = 0;
  ...
  for (i = 0; i < 120; i++) {
    colSum = 0.0;
    for (idx_global = 0; idx_global < 120; idx_global++) {
      colSum += std::abs(H[idx_global + 120 * i]);
    }
    H_infnrm = std::fmax(H_infnrm, colSum);
    f_infnrm = std::fmax(f_infnrm, std::abs(f[i]));
  }
  obj.ldq = 200;
  std::memset(&obj.Q[0], 0, 40000U * sizeof(double));
  std::memset(&obj.jpvt[0], 0, 321U * sizeof(int));
  obj.mrows = 0;
  obj.ncols = 0;
  obj.minRowCol = 0;
