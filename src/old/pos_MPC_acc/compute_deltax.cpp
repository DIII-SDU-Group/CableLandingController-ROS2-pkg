...
void compute_deltax(const double H[14400], struct_T *solution,
    ...
    if (qrmanager->ncols <= 0) {
      if (objective->objtype == 3) {
        ...
        if (qrmanager->mrows < 1) {
          ...
          if (qrmanager->mrows > 1) {
            ...
            for (lastColC = 2; lastColC <= nVar_tmp + 1; lastColC++) {
              double s;
              s = std::abs(cholmanager->FMat[(lastColC - 1) * 201]);
              if (s > smax) {
        ...
        cholmanager->regTol_ = std::fmax(
            std::abs(cholmanager->FMat[A_maxDiag_idx + 200 * A_maxDiag_idx]) *
        ...
        if (cholmanager->ConvexCheck) {
          ...
          do {
            exitg2 = 0;
            if (br <= nVar_tmp) {
              if (cholmanager->FMat[br + 200 * br] <= 0.0) {
        ...
        if (cholmanager->info != 0) {
          ...
          if (cholmanager->ndims != 0) {
            for (lastColC = 0; lastColC <= nVars + 1; lastColC++) {
              A_maxDiag_idx = lastColC + lastColC * 200;
              i = nVars - lastColC;
              for (ar = 0; ar <= i; ar++) {
                br = (lastColC + ar) + 1;
          ...
          i = cholmanager->ndims;
          for (br = 0; br < i; br++) {
            solution->searchDir[br] /= cholmanager->FMat[br + 200 * br];
          }
          nVars = cholmanager->ndims;
          if (cholmanager->ndims != 0) {
            for (lastColC = nVars; lastColC >= 1; lastColC--) {
              A_maxDiag_idx = (lastColC - 1) * 200;
      ...
      case 5: {
        ...
        if (qrmanager->mrows != 0) {
          ...
          br = 0;
          i = (nullStartIdx_tmp + 200 * (mNull_tmp - 1)) + 1;
      ...
      default: {
        ...
        case 3: {
          ...
          lastColC = 200 * (mNull_tmp - 1);
          for (cr = 0; cr <= lastColC; cr += 200) {
            i = cr + 1;
            i1 = cr + mNull_tmp;
            if (i <= i1) {
        ...
        if (cholmanager->info != 0) {
          ...
          i = cholmanager->ndims;
          for (br = 0; br < i; br++) {
            memspace->workspace_double[br] /= cholmanager->FMat[br + 200 * br];
          }
          nVars = cholmanager->ndims;
          if (cholmanager->ndims != 0) {
            for (lastColC = nVars; lastColC >= 1; lastColC--) {
