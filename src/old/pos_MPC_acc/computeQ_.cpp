...
void computeQ_(f_struct_T *obj, int nrows)
{
  ...
  if (nrows >= 1) {
    ...
    std::memset(&work[0], 0, 200U * sizeof(double));
        ...
        if (lastv + 1 > 0) {
          ...
          if (lastc + 1 != 0) {
            ...
            for (n = idx; n <= i; n += 200) {
              ...
              iQR0 = div_nde_s32_floor((n - iaii) - 200, 200);
              work[iQR0] += c;
            }
          }
          if (!(-obj->tau[itau] == 0.0)) {
            ...
            for (idx = 0; idx <= lastc; idx++) {
              ...
              if (c != 0.0) {
                ...
                i1 = lastv + iQR0;
                for (n = i; n <= i1 + 200; n++) {
                  obj->Q[n - 1] += obj->Q[((iaii + n) - iQR0) - 201] * c;
                }
              }
              iQR0 += 200;
            }
