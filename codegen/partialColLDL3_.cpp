                     ...
                     double REG_PRIMAL)
{
  ...
  for (k = 0; k < 48; k++) {
    ...
    if ((NColsRemain != 0) && (k != 0)) {
      ix = LD_offset + k;
      i1 = obj->ldm * (k - 1) + 1;
      for (iac = 1; lda < 0 ? iac >= i1 : iac <= i1; iac += lda) {
  ...
  for (int j{48}; j <= i; j += 48) {
    ...
    for (k = j; k <= i1; k++) {
      m = ia0 - k;
      offsetColK = (LD_offset + LDimSizeP1 * k) - 1;
      ...
      if (m != 0) {
        ix = 0;
        i2 = (k + obj->ldm * 47) + 1;
        for (iac = FMat_offset; lda < 0 ? iac >= i2 : iac <= i2; iac += lda) {
          i3 = (iac + m) - 1;
          for (idx = iac; idx <= i3; idx++) {
            subRows = (offsetColK + idx) - iac;
            obj->FMat[subRows] = obj->FMat[subRows] + obj->workspace_[idx - 1] *
                                                          -obj->workspace2_[ix];
          }
          ix++;
        }
