  ...
  if (solution->state >= 0) {
    ...
    if (solution->maxConstr > maxConstr_new) {
      phaseone(H, f, solution, memspace, workingset, qrmanager, cholmanager,
               objective, b_options, &runTimeOptions);
      if (solution->state != 0) {
        solution->maxConstr =
            WorkingSet::maxConstraintViolation(workingset, solution->xstar);
        if (solution->maxConstr > maxConstr_new) {
    ...
    if (guard1) {
      ...
      do {
        exitg1 = 0;
        if (kstr < 8) {
          if (cv[static_cast<unsigned char>(b_options->SolverName[kstr])] !=
      ...
      if (b_bool && (solution->state != -6)) {
        ...
        while ((solution->iterations < runTimeOptions.MaxIterations) &&
          ...
          phaseone(H, f, solution, memspace, workingset, qrmanager, cholmanager,
                   objective, b_options, &runTimeOptions);
          iterate(H, f, solution, memspace, workingset, qrmanager, cholmanager,
                  objective, b_options->ObjectiveLimit,
                  b_options->StepTolerance, runTimeOptions.MaxIterations,
